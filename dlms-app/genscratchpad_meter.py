...
# genscratchpad_meter.py - A tool to generate encrypted scratchpad contents for smart meter firmware update
#
# Requires:
#   - Python 3 v3.2 or newer
...
class InFile(object):
    ...
    def __init__(self, file_spec = None, data = None,
        ...
        if data:
            ...
            self.area_id, filename = self.parse_file_spec(file_spec)

            # Read data from file.
            memory = hextool.Memory()

            # Meter firmware header in app is:
            #typedef struct __attribute__ ((__packed__))
            #{
            #    uint32_t magic; // 0xACB3D8C9
            #    uint32_t status;
            #    uint32_t size;
            #    uint32_t rfu;
            #} meter_firmware_header_t;

            # Write our meter firmware header
            firmware_header = struct.pack("<4I",
                0xACB3D8C9, # Magic
                0xFFFFFFFF, # Status to unprocessed
                os.path.getsize(filename), # File length
                0xFFFFFFFF) # rfu_1
            memory += firmware_header

            hextool.load_binary(memory, filename=filename, offset=len(firmware_header))
    ...
    def parse_file_spec(file_spec):
        ...
        try:
            ...
            fields = file_spec.split(":", 1)
            if fields.__len__() == 2:
                area_id, filename = fields
            else:
                raise ValueError("invalid input file specification (wrong number of fiels): (area_id:filename)"
                             "'%s'" % file_spec)
            ...
            raise ValueError("invalid input file specification (wrog value type): (area_id:filename) "
                             "'%s'" % file_spec)

        return area_id, filename
...
def create_argument_parser(pgmname):
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog =
            ...
            "      filename                binary file for meter\n")
