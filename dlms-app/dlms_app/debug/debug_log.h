 ...
 * \brief Macro to retrieve a string from the log level.
 */
#define DEBUG_LVL_TO_CHAR(level) LVL_CHAR_##level
...
{ \
    ...
    UartPrintTrace(DEBUG_LVL_TO_CHAR(level), DEBUG_LOG_MODULE_NAME, (fmt), ##__VA_ARGS__) : \
    (void)NULL; \
} while (0)

/**
 * \brief   Set of macros that avoid to pass the full log level
 * \param   fmt
 *          Format of the log
 * \param   ...
 *          The list of parameters
